      ...
      >
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <button onClick={onBack} className="brutalist-button px-6 py-3 text-sm md:w-auto w-full">
            <ArrowLeft className="w-5 h-5 mr-2" /> NEW ANALYSIS
          </button>
        </div>
        ...
        <div className="space-y-6">
          {results.candidates.length > 0 ? (
            ...
            <div className="brutalist-card p-10 text-center">
              <h3 className="text-xl font-black tracking-widest uppercase mb-3">No candidate evidence found</h3>
              <p className="text-sm text-gray-600 mb-6">
                The files were processed, but none contained readable candidate text. Try a text-based PDF or DOCX.
              </p>
              <button onClick={onBack} className="brutalist-button px-6 py-3 mx-auto">
                <ArrowLeft className="w-5 h-5 mr-2" /> TRY DIFFERENT FILES
              </button>
            </div>
          )}
        </div>
