...
// The browser calls the same-origin BFF. It owns the upstream timeout, retry,
// and best-effort cache policy so no infrastructure detail reaches the client.
...
export const useScreeningStore = create<ScreeningState>((set, get) => ({
  ...
  processResumes: async () => {
    ...
    try {
      ...
      // The BFF has a 60-second Vercel budget and returns a typed 504 when the
      // upstream cannot recover within that limit.
      ...
      const msg = err instanceof Error
        ? (err.name === 'AbortError'
            ? 'The screening service did not respond in time. Please retry in a moment.'
